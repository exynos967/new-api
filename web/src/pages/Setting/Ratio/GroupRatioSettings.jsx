...
function verifyGroupDisplayJSON(value) {
  ...
  try {
    ...
    return (
      ...
      Object.values(parsed).every((item) => typeof item === 'boolean')
    );
  } catch {
    return false;
  }
      ...
      <Form.Section text={t('分组JSON设置')}>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('模型广场展示分组')}
              placeholder={t('为一个 JSON 文本，键为分组名称，值为是否展示')}
              extraText={t(
                '模型广场展示分组配置，格式为 JSON 字符串，例如：{"vip": true, "test": false}，只有值为 true 的分组会在模型广场展示，false 或未配置的分组不展示',
              )}
              field={'group_ratio_setting.group_display'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
              stopValidateWithError
              rules={[
                {
                  validator: (rule, value) => verifyGroupDisplayJSON(value),
                  message: t('必须是合法的 JSON 对象，且值必须为布尔值'),
                },
              ]}
              onChange={(value) =>
                setInputs((prev) => ({
                  ...prev,
                  'group_ratio_setting.group_display': value,
                }))
              }
            />
          </Col>
